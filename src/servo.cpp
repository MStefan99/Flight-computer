...
void servo::enable(uint8_t channel) {
	...
	if (pin & 0x1) {                                               // Odd pin
		PORT_REGS->GROUP[0].PORT_PMUX[pin / 2] =
		    (PORT_REGS->GROUP[0].PORT_PMUX[pin / 2] & 0xf) | (channel == 3 ? PORT_PMUX_PMUXO_F : PORT_PMUX_PMUXO_E);
	} else {  // Even pin
		PORT_REGS->GROUP[0].PORT_PMUX[pin / 2] =
		    (channel == 2 ? PORT_PMUX_PMUXE_F : PORT_PMUX_PMUXE_E) | (PORT_REGS->GROUP[0].PORT_PMUX[pin / 2] & 0xf0);
	}
}
...
void servo::setChannel(uint8_t channel, int16_t angle) {
	...
	getTimer(channel)->TCC_CCBUF[getTimerChannel(channel)] = util::clamp(static_cast<int>(angle), -1500, 1500) / 2 + 1500;
}
