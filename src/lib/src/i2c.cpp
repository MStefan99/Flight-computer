...
extern "C" {
	...
	void I2C_Handler() {
		...
		if (SERCOM_REGS->I2CM.SERCOM_INTFLAG & SERCOM_I2CM_INTFLAG_ERROR_Msk ||
			...
			if (!regAddrWritten) {
				SERCOM_REGS->I2CM.SERCOM_DATA = transfer.regAddr;
				regAddrWritten = true;
			} else {
				streamIn(transfer.devAddr, transfer.buf, transfer.len);
				SERCOM_REGS->I2CM.SERCOM_INTENCLR = SERCOM_I2CM_INTENSET_MB(1);
...
void i2c::init() { // GCLK config
	...
	// SERCOM config
	SERCOM_REGS->I2CM.SERCOM_BAUD = 33;  // 100KHz
...
static void streamOut(uint8_t devAddr, uint8_t* buf, uint8_t len) {
	...
	dma::DESCRIPTOR_TABLE[DMA_CH_I2C_TX].DMAC_DSTADDR = (uint32_t) & SERCOM_REGS->I2CM.SERCOM_DATA;
	DMAC_REGS->DMAC_CHCTRLA = DMAC_CHCTRLA_ENABLE(1);

	SERCOM_REGS->I2CM.SERCOM_ADDR = SERCOM_I2CM_ADDR_ADDR(devAddr << 1u)
...
static void streamIn(uint8_t devAddr, uint8_t* buf, uint8_t len) {
	...
	dma::DESCRIPTOR_TABLE[DMA_CH_I2C_RX].DMAC_SRCADDR = (uint32_t) & SERCOM_REGS->I2CM.SERCOM_DATA;
	dma::DESCRIPTOR_TABLE[DMA_CH_I2C_RX].DMAC_DSTADDR = (uint32_t)(buf + len);
	DMAC_REGS->DMAC_CHCTRLA = DMAC_CHCTRLA_ENABLE(1);

	SERCOM_REGS->I2CM.SERCOM_ADDR = SERCOM_I2CM_ADDR_ADDR(devAddr << 1u | 0x1)
					| SERCOM_I2CM_ADDR_LEN(len)
					| SERCOM_I2CM_ADDR_LENEN(1);
}
...
static void startTransfer() {
	...
	switch (transfer.type) {
		...
		case I2CTransferType::WriteRead:
			SERCOM_REGS->I2CM.SERCOM_INTENSET = SERCOM_I2CM_INTENSET_MB(1);
			SERCOM_REGS->I2CM.SERCOM_ADDR = SERCOM_I2CM_ADDR_ADDR(transfer.devAddr << 1u);
			break;
	}
}
