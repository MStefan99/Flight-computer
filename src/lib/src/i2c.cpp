...
void i2c::init() {                                                         // GCLK config
	...
	// PORT config
	PORT_REGS->GROUP[0].PORT_WRCONFIG = PORT_WRCONFIG_PINMASK((0x1 << 8u) | (0x1 << 9u)) | PORT_WRCONFIG_PMUXEN(1)
	                                  | PORT_WRCONFIG_PMUX(MUX_PA08D_SERCOM2_PAD0) | PORT_WRCONFIG_WRPMUX(1)
...
void i2c::write(
    ...
    uint8_t* buf,
    uint8_t  size,
    void     (*cb)(bool, const dma::I2CTransfer&),
    bool     littleEndian
) {
	auto transfer {
	  dma::I2CTransfer {
